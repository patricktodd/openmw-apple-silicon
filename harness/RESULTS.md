...
- Same tree with `applyClipPlanes` as a macro in `lib/core/vertex.h.glsl` (the `clipPlaneN` uniforms declared there through the same `@foreach`): zero GL errors, exit 0, screenshots match this branch's core runs (Vivec 0.3 %, Seyda 2.5 %). A plain function definition in that header does not link ("duplicate definition of function 'applyClipPlanes'"), since the header is included by several linked units (`-mr5502-core-inline`). The experiment hard-codes two planes and omits the GLES conditional branch.
